...
A separate sales-staff surface at `sales.shopbellasxv.com` that lets stylists prepare for appointments, log outcomes, build and sign quotes on the spot, add party participants, and clock in/out as an active-app signal. The admin app at `admin.shopbellasxv.com` stays the source of truth for business config, pipeline-wide reporting, and payments. Sales staff do not log into admin.

## Goal

Stylists open `sales.shopbellasxv.com` on a phone or iPad, type a PIN, clock in to declare "I am actively working the app", and see only what they need to run today's floor: today's appointments, drill-down detail, status quick-actions, dress try-on log, on-the-spot quote and signature capture, invoice conversion, and a guided "add participant" flow. Schedule and time-off live behind the same PIN. The admin app sees what sales does (notes, status changes, signatures, lead/contact views, and active app sessions) through the existing activity/audit surfaces so the owner has one timeline.
...
- **Commission-era clock-in is not a GPS/payroll control.** Sales reps are currently 100% commission, so clock-in means "this rep is using the app now." Do not require GPS/geofence proximity to clock in. Keep the existing attendance/clock tables as the session record, but treat them as activity/accountability data until hourly payroll rules become a real requirement.
- **Lead/contact viewing needs owner visibility.** The owner wants to know whether reps are looking at leads and contacts, not just whether they wrote notes. Record lightweight sales activity events when a rep opens a lead/event, appointment detail, contact detail, or sales-safe search result. The event payload should carry ids, route/surface, and timestamps, not private note text or financial fields.
- **Geofence remains a future/hourly-payroll control, not the commission-mode default.** The existing schema can store `(lat, lng, radius_m)` per location, and any future location-required mode should keep the server-side circular haversine check. For today's commission-only sales model, the clock-in handler must allow app sessions without GPS proximity. Client-supplied coordinates are inputs, never authority, and the OpenHRApp 0.005-degree client bounding box is not the model.
