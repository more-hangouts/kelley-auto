/*!
 * Kelley Autoplex Fit Prep Tool — embeddable IIFE
...
(function (window, document) {
  ...
  if (window.KelleyFitPrepTool && window.KelleyFitPrepTool._initialized) {
    return;
  }
  ...
  //     appointment-note field via KelleyBookingWidget.setNote(). Kept as
  ...
  // ---------------------------------------------------------------------
  // Size chart — Kelley Autoplex internal reference chart
  // ---------------------------------------------------------------------
  // This is Kelley Autoplex's own working reference for prep estimates, not a
  ...
  // point only.
  //
  // Customer-facing copy must say "Kelley Autoplex reference formalwear chart"
  ...
  // ---------------------------------------------------------------------
  var CHART_SOURCE = "Kelley Autoplex reference formalwear chart";
  ...
  var config = {
    containerId: 'kelley-fit-prep-tool',
    apiBaseUrl: '',
  };
  ...
  function buildSummary(data, sizing) {
    if (!sizing) return '';
    var lines = ['Fit Prep Summary (Kelley Autoplex)'];
  ...
  function saveSummary(summary) {
    ...
    if (window.KelleyBookingWidget && typeof window.KelleyBookingWidget.setNote === 'function') {
      try { window.KelleyBookingWidget.setNote(summary); } catch (e) { /* ignore */ }
    }
  }
  ...
  function clearSummary() {
    ...
    if (window.KelleyBookingWidget && typeof window.KelleyBookingWidget.setNote === 'function') {
      try { window.KelleyBookingWidget.setNote(''); } catch (e) { /* ignore */ }
    }
  }
...
`;
  ...
  function renderResult() {
    ...
    if (!s) {
      ...
      if (s.offChart) {
        wrap.appendChild(el('p', { className: NS + '-subtitle' }, [
          'Your measurements are toward the upper end of our reference chart. Kelley Autoplex carries designers with extended sizing, so your stylist will help you find the right fit in store.',
        ]));
      } else {
      ...
      wrap.appendChild(edu(
        ...
        'Kelley Autoplex carries multiple designers (Morilee, House of Wu, and others), and each designer uses their own chart. ' +
    ...
    submitProfileWithToken(state.token, payload).then(function (res) {
      ...
      if (res.status === 404) {
        state.handoff.errorMsg = (
          ...
          '(210) 251-3644 and we will save your answers for you.'
        );
      } else if (res.status === 409) {
        state.handoff.errorMsg = (
          'This appointment is no longer active. Please call us at ' +
          '(210) 251-3644 if you need to rebook.'
        );
  ...
  function saveProfileId(id) {
    ...
    if (window.KelleyBookingWidget &&
        typeof window.KelleyBookingWidget.setBoutiqueExperienceProfileId === 'function') {
      try { window.KelleyBookingWidget.setBoutiqueExperienceProfileId(id); }
      catch (e) { /* ignore */ }
    }
  }
  ...
  function clearProfileId() {
    ...
    if (window.KelleyBookingWidget &&
        typeof window.KelleyBookingWidget.setBoutiqueExperienceProfileId === 'function') {
      try { window.KelleyBookingWidget.setBoutiqueExperienceProfileId(null); }
      catch (e) { /* ignore */ }
    }
  }
